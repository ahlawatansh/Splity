...
const EXAMPLE_DB_FILE = path.join(process.cwd(), 'data_store.example.json');
...
function loadDb() {
  try {
    ...
    } else if (fs.existsSync(EXAMPLE_DB_FILE)) {
      const content = fs.readFileSync(EXAMPLE_DB_FILE, 'utf-8');
      dbData = JSON.parse(content);
      saveDb();
    } else {
      initSeedData();
